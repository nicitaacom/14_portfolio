...
// zustand middleware factories (zustand/middleware) wrap a store creator function - the same
// (set, get) => ({...}) shape a selector callback has, but this is defining the store, not
// selecting a slice from it. Not store hooks, so never flag a call to one of these.
const ZUSTAND_MIDDLEWARE_NAMES = new Set(["persist", "subscribeWithSelector", "devtools", "combine", "redux", "createJSONStorage"])
...
module.exports = {
  "zustand-no-selector": {
    meta: {
      ...
      docs: {
        description:
          "disallow zustand selector-style usage (useStore(state => state.value)) - destructure the whole store instead",
      },
      schema: [],
      messages: {
        noSelector:
