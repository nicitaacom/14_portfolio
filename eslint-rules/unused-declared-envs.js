...
//   3. that same library also takes it as an argument - hand it over, and the warning goes
...
// commented line in the local dotenv file, keeps the record without the ghost completion. It needs
// no suppression either: parseDeclarations strips // comments before it matches, so the line stops
// being a declaration and this rule goes quiet on its own.
//
// 3 is the settlement app/libs/supabaseClient.ts already uses - createClientComponentClient takes
// supabaseUrl and supabaseKey as arguments, so NEXT_PUBLIC_SUPABASE_URL and
// NEXT_PUBLIC_SUPABASE_ANON_KEY are read by name in a source file and never reach this rule.
...
// edit it needs - a comment-out, a suppression, an argument, or a deletion.
const SETTLEMENTS =
  ...
  "value as a commented line in your local dotenv file. A commented declaration is stripped before " +
  ...
  "supabaseUrl: process.env.NEXT_PUBLIC_SUPABASE_URL, supabaseKey: process.env.{{name}} }), and the " +
  "warning goes on its own\n" +
  "4. read by CI/CD and by a development-only command, the way CHROMATIC_PROJECT_TOKEN is read by a " +
  "workflow and by `pnpm storybook` - keep it, and suppress with that workflow file named in the " +
  "reason\n" +
...
// each non-comment line, same parse envs-order.js runs. Only the opening sentence of the report
