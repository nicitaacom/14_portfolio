...
export async function selectDBUTMStatsAction(): Promise<UTMAggregatedStats | string> {
  try {
    ...
    const recentVisits = entries.filter(entry => new Date(entry.created_at) >= thirtyDaysAgo).length
