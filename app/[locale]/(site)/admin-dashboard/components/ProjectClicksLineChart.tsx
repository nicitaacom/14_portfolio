...
function CustomTooltip({
  ...
  return (
    <div className="rounded-[4px] border border-brass/40 bg-steel-deep px-xs py-xs shadow-[0_8px_24px_rgba(0,0,0,0.4)]">
...
function CustomDot({
  ...
  const strokeColor = isLatest ? "var(--3d-dot-c-9b9b9b)" : "var(--3d-dot-c-6b8cff)"
...
export function ProjectClicksLineChart({ projectName, timeline, timelineMode }: ProjectClicksLineChartProps) {
  ...
  if (!timeline.length) {
    return (
      <p className="py-lg text-center text-sm text-secondary-foreground">{t("noProjectClickData")}</p>
    )
  }
  ...
  return (
    <div className="flex min-w-0 flex-col gap-xs">
      {/* Header stats */}
      <div className="grid gap-xs rounded-[2px] border border-brass/40 bg-steel-deep p-xs tablet:grid-cols-[minmax(0,1fr)_auto]">
        ...
        <div className="grid grid-cols-3 gap-xs">
          <div className="rounded-[2px] border border-brass/40 bg-steel-deep px-xs py-xs">
      ...
      {/* Chart */}
      <div className="rounded-[2px] border border-brass/40 bg-steel-deep p-xs">
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={timeline} margin={{ top: 16, right: 12, bottom: 0, left: -8 }}>
            <defs>
              <linearGradient id="areaGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="var(--3d-dot-c-6b8cff)" stopOpacity={0.22} />
                <stop offset="60%" stopColor="var(--3d-dot-c-6b8cff)" stopOpacity={0.06} />
                <stop offset="100%" stopColor="var(--3d-dot-c-6b8cff)" stopOpacity={0} />
              </linearGradient>
            </defs>

            <CartesianGrid
              strokeDasharray="2 6"
              stroke="var(--3d-dot-c-252525)"
              vertical={false}
            />

            <XAxis
              dataKey="bucket_label"
              tick={{ fill: "var(--3d-dot-c-555)", fontSize: 10, fontFamily: "inherit" }}
              tickLine={false}
              axisLine={false}
            ...
            <Tooltip
              ...
              cursor={{ stroke: "var(--3d-dot-c-3a3a3a)", strokeWidth: 1, strokeDasharray: "4 4" }}
            />

            {peakPoint && (
              <ReferenceLine
                x={peakPoint.bucket_label}
                stroke="var(--3d-dot-c-3a3a3a)"
                strokeDasharray="3 5"
                label={{ value: "↑ peak", position: "insideTopRight", fill: "var(--3d-dot-c-4a4a4a)", fontSize: 9 }}
              />
            )}
            ...
            <Area
              ...
              activeDot={{ r: 5, fill: "var(--3d-dot-c-1c1c1c)", stroke: "var(--3d-dot-c-6b8cff)", strokeWidth: 2 }}
