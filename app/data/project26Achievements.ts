...
export const project26Achievements: GroupedAchievements = {
  "🎨 Frontend": {
    ...
    "Created responsive website from Figma using Tailwind": undefined,
  },
  "🧩 Backend": {
    "Created API routes with documentation and server actions to keep sensitive data secure": undefined,
    "Used ISR for SSG pages on /support/[ticketId] route": undefined,
    "Created a closed GitHub repository for Git merges and backups": undefined,
    "Implemented AWS S3 bucket to save images that user uploads using AWS SDK": undefined,
    "Created backup feature for project that allows backing up tables and storages (Supabase buckets) even on Vercel with 60s timeout and 2GB memory":
      undefined,
    "Created UTM params - so I can see a dashboard with animations that shows statistics": undefined,
    "Maintained project - it was broken after 2 years of inactivity - fixed UI and UX issues, fallback, AI errors":
      undefined,
  },
  "🖌️ Figma & responsiveness": {
  ...
  "🧭 UI/UX/CX": {
    "Created UI to upload custom food images that user uploads with AWS S3 Bucket": undefined,
    "Created Admin CMS - so admin can add / edit / delete food": undefined,
    "Created HR CMS - so HR can add / edit / delete job posting": undefined,
    'Created Chef CMS - so chef can manage orders, also deliveryman can click on "Show map" to see where order should be delivered using Google Maps API':
      undefined,
