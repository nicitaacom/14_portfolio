/**
 * Are the API keys in the local dotenv file still valid?
 *
 * Run it with `pnpm test:keys`. It is not a unit test - every "live" name here sends a real request to
 * Supabase, Upstash, Telegram and Resend, so it also fails while one of those has an outage. That is
...
// this project names its example file .env.local.example, not .env.example
...
describe("every name declared in the project is in the registry", () => {
  ...
  it(`app/utils/checkKeys.ts lists every name in ${EXAMPLE_FILE}`, () => {
    const exampleNames = readNames(EXAMPLE_FILE, DOTENV_NAME_PATTERN)
    assert.ok(exampleNames, `${EXAMPLE_FILE} was not found next to this test`)

    const missing = [...exampleNames].filter(name => !registryNames.has(name))
    assert.deepEqual(missing, [], `declared in ${EXAMPLE_FILE} but missing from app/utils/checkKeys.ts: ${missing.join(", ")}`)
  })
...
describe("every key in the local dotenv file still works", () => {
  let report
