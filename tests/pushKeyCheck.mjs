 ...
 *
 *   1. It never stops a push. A third party having a 500 for ten minutes is not a reason to block
 *      your work. It prints the names and steps aside.
...
const daysSinceLastRun = readDaysSinceLastGreenRun()
if (daysSinceLastRun !== null && daysSinceLastRun < PUSH_CHECK_EVERY_DAYS) {
  const daysRounded = Math.floor(daysSinceLastRun)
...
process.stderr.write("pre-push: checking every API key in the local dotenv file\n")
